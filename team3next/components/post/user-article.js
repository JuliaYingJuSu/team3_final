...
export default function UserArticle({ userinfo }) {
  const { auth } = useContext(AuthContext);
  const [imgs, setImgs] = useState([]);
  const [comments, setComments] = useState([]);
  const contentParagraphs = userinfo.post_content.split("\n");
            ...
            <div className="ms-3 text-start">
              ...
              <p className="icon-tag">
                <a className="ms-2 " href="#" style={{ color: "#666666" }}>
                  {userinfo.food_tag_name}
                </a>
              </p>
              <p className="fs12 mb-2">{userinfo.create_date.substr(0, 10)}</p>
            </div>
            <hr />
            {comments &&
              comments.map((v, i) => {
